...
**A second iSCSI drop (2026-07-02) left the primary stuck, and CNPG never failed over — for 2 days.** Another iSCSI session drop on `luffy` left the primary (`cnpg-cluster-2`) pointing at storage that no longer existed. Querying Postgres directly gave a clear, fast error, but CNPG's own liveness check (`/healthz`) kept passing the whole time — 0 restarts in 2 days — while readiness (`/readyz`) correctly failed the entire time. That means `/healthz` only checks whether the process is alive, not whether it can actually read or write data, so CNPG never saw a reason to fail over on its own. Fix: added a Prometheus alert, `PodNotReadyExtended`, that watches whether a pod is marked "ready" in Kubernetes for 15+ minutes, independent of restart count and independent of CNPG entirely — so if this happens again, we get notified instead of finding out days later. We tested the actual failover path afterward by manually deleting the stuck primary pod: CNPG promoted the replica and had the cluster fully healthy again in under a minute, confirming the failover mechanism itself works fine — it just needs a clear signal to act on, which this specific "stuck, not crashed" failure never gave it. For now this stays a human-in-the-loop step (alert fires, we decide, we act) rather than fully automated; auto-deleting the pod the moment the alert fires is a reasonable next step but carries its own risk if the alert ever fires on something that's merely slow rather than truly stuck, so it's deferred rather than rushed.
